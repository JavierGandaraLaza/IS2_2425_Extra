	...
	public static void main(String[] args) { //WMC +1
		...
		while (true) { //WMC +1, CCog +1
			...
			case NUEVA_VENTA: //WMC +1
				...
				try {
					if (!tienda.anhadeVenta(dni, importe)) { //WMC +1, CCog +3 (nesting=2)
						mensaje("ERROR", "El vendedor no existe");
					}
				} catch (DataAccessException e) { //WMC +1, CCog +3 (nesting=2)
			...
			case VENDEDOR_DEL_MES: //WMC +1
				try {
					...
					for (Vendedor v : vendedores) { //WMC +1, CCog +3 (nesting=2)
						if (v.getTotalVentas() > maxVentas) { //WMC +1, CCog +4 (nesting=3)
					...
					msj = "";
					for (Vendedor vn : resultado) { //WMC +1, CCog +3 (nesting=2)
						msj += vn.getNombre() + "\n";
					}
				...
				try {
					...
					Collections.sort(vendedores, new Comparator<Vendedor>() {
						public int compare(Vendedor o1, Vendedor o2) {
							if (o1.getTotalVentas() > o2.getTotalVentas()) //WMC +1, CCog +4 (nesting=3)
								return -1;
					...
					msj = "";
					for (Vendedor vn : vendedores) { //WMC +1, CCog +3 (nesting=2)
